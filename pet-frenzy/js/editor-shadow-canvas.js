...
function initShadowBigCanvasOnce(){
  ...
  _shadowBigReceiver = ShadowLayoutReceiver.create(_shadowBigCanvas, { stageId: '_shadow_compose' });
  _shadowBigReceiver.attachPointerEvents(drawShadowBigCanvas); // 直接在大畫布上拖曳/縮放商品與主持人（點選會透過 LC_ACTIVE_CHANGED 自動同步回左側素材清單的選取高亮）

  /* 底色吸取（毛孩衝蝦米）：不再預設載入 backgrounds/1200bg.jpg 這張固定模擬背景圖，
     預設直接用 S.seedHex 純色填底，換背景色這裡也會跟著換。
     shadow-compose-bg-file 這個手動上傳按鈕還留著，當使用者想暫時換張圖預覽時可以蓋掉純色，
     沒有上傳就一律用底色吸取的顏色（見 drawShadowBigCanvas()）。 */
...
function drawShadowBigCanvas(){
  ...
  if(_shadowBigSimBg){ _shadowBigCtx.drawImage(_shadowBigSimBg,0,0,1200,1200); }
    ...
    /* 底色吸取（毛孩衝蝦米）：沒有手動上傳暫用背景圖時，用 S.seedHex 純色填底 */
    _shadowBigCtx.fillStyle = (window.S && S.seedHex) || '#d8d8d8';
    _shadowBigCtx.fillRect(0,0,1200,1200);
  }
  _shadowBigReceiver.drawStage(_shadowBigCtx); // 圓柱舞台（毛孩衝蝦米）：畫在商品/主持人最下方
...
function exportShadowComposite(){
  ...
  /* 2026-07-28 修正：匯入工單時會呼叫 ShadowEditor.enterPending()（暫存模式，
     版型/素材變更先不廣播給各版位），但整個專案裡從來沒有任何地方呼叫對應的
     ShadowEditor.commit() 讓它離開暫存模式——這裡（使用者確認匯出）是真正的
     「使用者確認了，可以正式套用」的時間點，在這裡呼叫 commit()，之後
     版型切換／素材調整才會正常即時廣播出去，不會卡在暫存模式出不來。 */
  if(window.ShadowEditor && typeof window.ShadowEditor.commit === 'function'){
    window.ShadowEditor.commit();
  }
  ...
  /* 合成：舞台先貼（最底層，跟商品/主持人一起攤平匯出，各版位不用再各自畫一次），
     再貼陰影，最後疊照片 */
  var outCv = document.createElement('canvas');
  outCv.width = 1200; outCv.height = 1200;
  var octx = outCv.getContext('2d');
  _shadowBigReceiver.drawStage(octx, { skipSelection: true }); // 圓柱舞台（毛孩衝蝦米），匯出時不能把選取框一起烤進去
