...
/* 給直播間FL「以Logo底色填滿」模式專用──真的去吸Logo素材本身的底色，
   ...
   ★ 明確依副檔名判斷（不是靠透明度間接猜）：
     - PNG：固定回傳白色。PNG 通常是去背圖，即使背景沒去乾淨、填了不透明的顏色，
       也大多是想要「無底色」的效果，統一給白色最安全，不去猜測那個殘留色。
...
/* ── 側邊清單樣式的單一 slot：跟 1200 陰影編輯器的素材清單、以及雙logo並排版本
   同一套「有圖/無圖兩態」邏輯——有圖顯示縮圖＋右上角×刪除，沒有圖顯示虛線框＋
   「+」，點下去觸發上傳。這裡維持單一logo（不做A/B並排），只是把原本陽春的
   純檔案選取欄位，換成跟其他圖層清單一致的外觀跟操作方式。 ── */
function logo2SyncSlotUI(){
  ...
  if(_logo2RawSrc){
    ...
    slot.innerHTML =
      ...
      '<span class="logo2-slot-label">Logo2</span>' +
...
/* 移除目前的 logo2 素材：清掉這個編輯面板裡「尚未合成的原圖」狀態＋位移/縮放，
   也一併清掉已經合成套用到版位的 S.imgs.logo2（如果先前按過「確認並套用」的話）——
   不然使用者在這裡刪除，會以為刪掉了，結果版位上舊的合成圖其實還留著，兩邊要一起清乾淨。 */
function logo2RemoveLogo(){
  ...
  _logo2Shape = 'wide'; // 沒有素材了，形狀重設回預設值
  ...
  if(S.imgs.logo2 && typeof clearLogo2 === 'function') clearLogo2();
}

/* ── 載入素材（使用者手動上傳新圖時呼叫，會重設縮放位移） ── */

/* ── 方形/長型：預設自動判斷（見 logo2DetectShape），這裡讓使用者手動覆蓋 ──
   換一張新素材時（logo2LoadImageFromSrc／openLogo2Popup還原）都會重新自動判斷，
   並把 radio 選取同步回去，不會沿用上一張素材手動選過的結果。 */
...
function logo2SetShapeOverride(shape){
  ...
  if (!_logo2Img) { logo2SyncShapeRadioUI(); return; } // 還沒上傳素材，先把 radio 同步回目前狀態，不做任何事
  _logo2Shape = shape;
  logo2ResizeCanvasToShape();
  drawLogo2BigCanvas();
  logo2UpdateFlPreview();
...
function logo2UpdateHint(){
  ...
  /* 2026-07-29 跟 Iona 確認移除：操作說明文字（形狀判定＋拖曳/縮放/方向鍵說明）不再顯示，
     素材載入後這裡保持空白，避免跟畫布上的實際操作重複佔版面 */
  el.textContent = '';
}
...
/* ── 直播間FL(07_fl.html) 示意圖 ──────────────────────────────────────
   目的：讓使用者在「Logo2 編輯」popup 裡，確認/套用之前，就能看到這個logo2
   之後出現在 07_直播間FL 版位（純Logo版型＝版型L）時的實際樣子——固定白底、
   橘色邊框、膠囊造型（四角全圓，圓角＝高度的一半），跟 07_fl.html 的
   render()/drawVariantL() 用同一套規格算出來，兩邊視覺一致，不用各自維護一份。

   只在「直播間FL ICON」欄位目前選的是 LOGO 時才顯示這塊示意圖。 */
var FL_PREVIEW = {
  W: 336, H: 120,
  BORDER_W: 3,
  BORDER_COLOR: '#EE4D2D',
...
/* 目前「直播間FL ICON」欄位是不是LOGO模式：下拉選單選「LOGO」，或 txt-fl
   文字打「logo」，兩種都算（雙重保險，跟 editor-utils.js 的 ccFl() 同一套判斷）。 */
...
/* 使用者在「Logo2 編輯」popup 裡選擇這個Logo的底色類型（二選一：白底＋橘框／
   吸Logo底色填滿），存進 S.flLogoBgMode，廣播給所有版位（07_fl.html據此決定
   背景怎麼畫），並立刻更新這裡的示意圖，讓使用者馬上看到差異。 */
...
/* 跟正式版位 07_fl.html 的 drawVariantL() 逐行對應同一套公式（原圖有色範圍＋
   Logo2畫布本身的縮放位移＋這裡的FL額外縮放位移），示意圖才會跟實際套用出來
   的結果一致，不會「示意圖看起來OK，套用到版位卻對不起來」。 */
