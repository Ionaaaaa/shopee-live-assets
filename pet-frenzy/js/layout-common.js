/*
  ...
  用法：在各版位 <script> 區塊最前面（比自己的內嵌 <script> 早）加一行：
    <script src="../js/layout-common.js?v=20260730a"></script>
    （這次開始每個版位都加上 ?v= 版本號，之後這份檔案更新，記得比照更新每個
    版位裡引用它的那一行——這份是「共用」檔案，沒有版本號的話瀏覽器快取很容易
    卡住舊版本，改了程式卻看起來像沒生效，跟這次「懸浮工具列改了但畫面沒變」
    是同一類問題，加版本號才能保證瀏覽器一定重新抓最新內容。）
*/

console.log('%c[layout-common.js] 版本確認：2026-07-30-v3（stage/overlay缺少宣告時不再ReferenceError）', 'background:#222;color:#0f0;font-weight:bold;padding:2px 6px;');

function font(size, w){ return w+' '+size+'px "ShopeeNoto","Noto Sans TC",sans-serif'; }

/* 十六進位色碼跟白色混合變淺（給底部遮罩「顏色跟著底色換」用，2026-07-29 跟 Iona 確認新增）。
   ratio 0~1，固定公式：跟白色以這個比例混合，數字越大越淺，不做亮/暗背景雙向判斷
   （不是跟文字/logo同一套「亮壓暗、暗提亮」邏輯——遮罩只需要「深＝底色本身，淺＝底色加白」）。
   刻意寫成獨立小函式、不呼叫 color-theme-engine.js，因為不是每個版位都一定有載入那份檔案。 */
function hexLighten(hex, ratio){
  if(!hex) return hex;
...
function logo2HeightRatioFor(shape, ratios){
  return shape === 'square' ? ratios.logo2HeightRatioSquare : ratios.logo2HeightRatioWide;
}

/* 依 host-scene-scale.js 查出這個版位「主持人圖層」第一次自動貼合時要再縮放多少倍。
   只影響 initHostPos 算出來的初始大小，使用者手動拖曳/縮放調整過之後不受影響。
   combo（選填）：目前版型 A/B/C/D。host-scene-scale.js 裡每個版位可以是：
     - 純數字：所有版型共用（向下相容舊寫法）
...
/* 依 host-scene-scale.js 裡的 HostSceneOffsetY 查出這個版位「主持人圖層」
   ...
   combo（選填）：目前版型 A/B/C/D，規則跟 getHostSceneScale 一致（純數字＝共用，物件＝可依版型細分）。
...
function drawMaskLayer(ctx){
  ...
  /* 顏色跟著底色吸取換色（2026-07-29 跟 Iona 確認新增）：深色＝目前的底色本身，
     淺色＝底色加白調淺一點點，不再用 mask-defaults.js 裡寫死的 color/lightColor。
     用 Object.create(m) 淺拷貝，只覆蓋顏色兩個欄位，不動到 mask-defaults.js 原始物件
     （height/dip/glowWidthRatio 等其他數字繼續照原本的版位設定，透過 prototype 鏈讀到）。 */
  if(typeof D !== 'undefined' && D.bgColor){
    m = Object.create(m);
    m.color = D.bgColor;
    m.lightColor = hexLighten(D.bgColor, 0.45);
  }
...
function updateOverlay(){
  ...
  // 2026-07-29 跟 Iona 確認簡化：原本「去背編輯／更換／刪除」三顆，改成「編輯／刪除」兩顆，
  // 「編輯」不再開去背工具，改成直接開 1200 商品/主持人陰影編輯畫布（跟側欄「陰影」按鈕開的是同一個 popup）。
  var bar=document.createElement('div');
  bar.className='sel-bar';
  bar.innerHTML=
    '<button class="sel-bar-btn" onclick="doAction(\'edit\')">'
    +'<svg viewBox="0 0 16 16"><path d="M2 14l2-5L12 1l3 3-8 8-5 2z"/><path d="M10 3l3 3"/></svg>編輯</button>'
...
function doAction(action){
  if(action==='edit'){
    console.log('[layout-common.js] doAction(edit) 被觸發，window.parent!==window：', window.parent!==window);
    if(window.parent!==window){
      window.parent.postMessage({type:'BN_OPEN_SHADOW'}, '*');
    }
...
/* ── Drag ──
   2026-07-30 補防護：這整段是「主持人/商品拖曳選取」互動，只有宣告了 stage/overlay
   的版位才需要（目前是 01/02/05/07）。09_tab.html 這種沒有可拖曳圖素的版位如果沒
   宣告這兩個變數，這裡直接噴 ReferenceError 會把後面的程式全部卡住——所以外層包一層
   typeof 判斷，沒有 stage 就整段跳過，不影響其他版位原本的行為。 */
