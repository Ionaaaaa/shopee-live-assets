...
/* 「直播間FL ICON」下拉選單選「不製作」時：
   - 側欄「FL 文案」輸入框整塊隱藏，不用讓使用者對著一個不會被匯出的欄位打字
   - 畫布區裡的 07_fl 畫布也隱藏，不佔畫面空間
   canvas-block-07_fl 只在 buildCanvasArea() 時建立一次，這裡只是切換 display，
   不用重新產生 iframe。
   呼叫時機：下拉選單onchange（handleFlSlotChange）、Excel匯入完成後、
   切換分頁後（applyTabData），以及初次建立畫布之後。 */
function updateFlCanvasVisibility(){
  ...
  var flBlock = document.getElementById('canvas-block-07_fl');
  if(flBlock) flBlock.style.display = isSkip ? 'none' : '';

  updateAssetList(); // 畫布區顯示/隱藏切換完了，左側素材清單要跟著同步
}
...
function broadcastText(){
  broadcastPayload({
    ...
    bgColor: S.seedHex, // 底色吸取（毛孩衝蝦米）：LPBN/縮圖/開播字卡/Tab 的純色底跟這顆走
    flBBgColor: S.seedHex, // 直播間FL純文案/商品版型背景色，改吃底色吸取的seedHex（原本是flBBgColorFor(theme)固定色）
    flBTextColor: (S.seedPalette && S.seedPalette.cMain) || flBTextColorFor(S.theme), // 改用seed算出來的對比文字色，算不出來才退回舊固定色
...
function broadcastFull(){
  broadcastPayload({
    ...
    /* 原圖＋縮放位移：直接給 07_fl.html 用「跟 Logo2 編輯畫布一樣的比例／位移」重算，
       FL 裡的 Logo 才會跟編輯畫布連動 */
    logo2Raw:   S.logo2Raw,
...
window.addEventListener('message', function(e){
  ...
  if(msg.type === 'BN_READY'){
    ...
    /* layout 懸浮 action bar 的「編輯」按鈕：跟側欄「陰影」按鈕開同一個 popup
       （見 layout-common.js doAction('edit') 的註解，2026-07-29 跟 Iona 確認的規格） */
