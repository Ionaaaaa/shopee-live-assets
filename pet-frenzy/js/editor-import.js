...
function setImportZoneSuccess(zoneId, on){
  ...
  zone.style.borderColor = on ? 'var(--accent)' : 'var(--border2)';
}
...
function onImportFilePicked(e, kind){
  if(kind === 'excel'){
    ...
    var rawFiles = Array.prototype.slice.call(e.target.files);
    var files = rawFiles.filter(function(f){ return /\.(png|jpe?g|webp)$/i.test(f.name); });
    var rejected = rawFiles.filter(function(f){ return files.indexOf(f) === -1; });
    if(rejected.length){
      console.warn('[import] 以下檔案副檔名不是 png/jpg/jpeg/webp，被跳過沒有匯入：', rejected.map(function(f){ return f.name; }));
      toast('有 '+rejected.length+' 個檔案副檔名不支援（只吃 png/jpg/jpeg/webp），已略過，請檢查瀏覽器 Console', 'err', 5000);
    }
    console.log('[import] 素材資料夾選到的檔案：', files.map(function(f){ return f.name; }));
...
function matchFileByAliases(files, aliases){
  ...
  for(var i=0;i<files.length;i++){
    ...
    for(var j=0;j<aliases.length;j++){
      ...
      if(!a) continue;
      if(base.indexOf(a) !== -1 || (a.length>=2 && base.length>=2 && a.indexOf(base)!==-1)) return files[i];
    }
  }
  return null;
}
...
function matchAndApplyLogoFiles(files, g, cb){
  g = g || {};
  var matchedCount = 0;
  var consumed = []; // 這次比對用掉的檔案，回傳給呼叫端排除，避免同一個檔案又被商品/主持人比對搶走
  console.log('[import][logo] g.logoName='+JSON.stringify(g.logoName)+'，可用檔案：', (files||[]).map(function(f){ return f.name; }));
  if(!files || !files.length){ window.__importedLogoFiles = []; if(cb) cb(0, consumed); return 0; }
  Object.keys(LOGO_ALIASES).forEach(function(key){
    var f = matchFileByAliases(files, LOGO_ALIASES[key]);
    if(f){ applyImageFile(f, key); matchedCount++; consumed.push(f); }
  });
  window.__importedLogoFiles = files; // 保留整包，供之後的 Logo 功能使用
  if(g.logoName){
    var pendingLogoFile = matchFileByAliases(files, [g.logoName]);
    console.log('[import][logo] 拿 g.logoName='+JSON.stringify(g.logoName)+' 去比對，結果：', pendingLogoFile ? pendingLogoFile.name : null);
    window.__pendingLogoMatch = { name: g.logoName, file: pendingLogoFile || null };
    if(pendingLogoFile){
      consumed.push(pendingLogoFile);
      ...
      reader.onload = function(ev){
        ...
        matchedCount++;
        console.log('[import][logo] S.logo2Raw 已寫入，等使用者在 Logo2 面板按確認/下一步才會真的套用到 S.imgs.logo2');
        if(cb) cb(matchedCount, consumed); // 一定要等讀完才回呼，不然 S.logo2Raw 還沒寫入 popup 就先開了，會看起來像沒讀到
      };
      reader.readAsDataURL(pendingLogoFile);
      return matchedCount; // 非同步分支：實際筆數會晚一點透過 cb 才知道，這裡先回傳目前值
    }
  }
  if(cb) cb(matchedCount, consumed);
  return matchedCount;
}
...
function confirmImport(){
  ...
  function afterExcel(groupData){
    ...
    /* 底色吸取（毛孩衝蝦米）：Excel「背景色」欄位→applySeedHex，
       LPBN/縮圖/開播字卡/FL Icon/Tab 的背景、公版文字色都會自動重算 */
    if(groupData.bgColor && window.ColorThemeEngine){
      var normalizedBg = window.ColorThemeEngine.normalizeHex(groupData.bgColor);
...
function parseWorkorderGroups(rows){
  ...
  var fieldMap = {'款式':'theme','主標':'main','副標':'sub','日期':'date','時間':'time','素材路徑':'hostPath','版型':'combo',
    '主持人':'host1Name','來賓':'host2Name','KOL':'host1Name','商品1':'product1Name','商品2':'product2Name','商品3':'product3Name','LOGO':'logoName',
    'FL文案(6字)':'flText','背景色':'bgColor','背景色碼':'bgColor','指定色號':'bgColor','指定色碼':'bgColor'};

  /* 2026-07-28（拿「毛孩衝蝦米/公版」實際工單檔案模擬驗證後修正，推翻同一天稍早的版本）：
     「製作內容文案」這個附加區塊（L14:P14 那一整塊）實際上是 L欄=標籤、M欄=值，
     不是原本以為的 M=標籤、N=值。舊邏輯會把 M20 殘留的標籤字樣「商品1」誤判成
     欄位名稱，把 N20 真正的 LOGO 名稱「蝦皮寵物展」誤存進 product1Name，導致
     LOGO 素材被商品比對搶走、真正的版型組合（在 M21）也從沒被讀到。這裡改成
     從 L欄 讀標籤、M欄 讀值。 */
  function isPlaceholderCaption(s){
    /* 模板裡「文案最多5字內」這類佔位提示字，使用者沒真的填寫時
       不該被當成真正的文案存進 flText */
    if(s === undefined || s === null) return true;
    s = String(s).trim();
    if(!s) return true;
    return /文案最多|字內|字元內|百分比例|請填|範例/.test(s);
  }

  rows.forEach(function(row, ri){
    var labelVal = row[11]; // L 欄 index 11（標籤）
    var valVal   = row[12]; // M 欄 index 12（值）
    /* 遇到「款式」，如果目前這組已經有資料了，代表是下一組新的開始 */
    if(labelVal === '款式' && Object.keys(current).length > 0){
      current = {};
      groups.push(current);
    }
    if(current && labelVal && valVal !== undefined && valVal !== null && valVal !== ''){
      var key = fieldMap[String(labelVal).trim()];
      if(key){
        var val = valVal;
        ...
        /* LOGO：工單下方「版型/商品/LOGO/案型」對照表裡剛好有一列的版型名稱就叫「LOGO」，
           那一列的 M/N 欄會被誤判成這裡的 LOGO 欄位，把正確的廠商Logo名稱覆蓋成 0/1 這種
           數字——真正的 LOGO 欄位值一定是名稱字串，不會是純數字，數字就略過不覆蓋 */
        if(key === 'logoName' && typeof val === 'number') return;
        /* 背景色：過濾「無/GD/請/指定」等無效填寫（同一份工單「其他需求」欄常見的占位字樣），
           填了才記錄，套用交給 afterExcel() 呼叫 applySeedHex()，不在這裡直接套用
           （這支函式只負責解析，不做DOM/廣播副作用，跟檔案開頭的設計原則一致） */
        if(key === 'bgColor' && /^(無|GD|請|指定)/.test(String(val).trim())) return;
        current[key] = String(val).trim();
      }
    }
    /* FL 版型／商品／文案：「直播間FL」這個跨兩列的合併儲存格實際在 L欄（不是M欄），
       2026-07-28 拿實際工單檔案模擬驗證後修正（推翻同一天稍早、根據錯誤欄位假設寫的版本）：
         - 第一列（L='直播間FL' 那一列）的 N 欄＝版型，是決定一切的主欄位，
           例如「LOGO」／「商品 + 案型5字內」／「純案型6字內」／「不製作」
           （M欄是固定字樣「版型」兩個字，只是視覺上的子標籤，不是資料）
         - 第二列（緊接在下面那一列）：
             N 欄＝版型是「商品 + 案型5字內」時，選的是哪個商品（商品1／商品2／商品3／禮物盒）；
                    版型是「LOGO」時，這欄填的是LOGO實際名稱，拿去跟素材資料夾比對用
             M 欄＝模板殘留標籤字樣（例如「商品1」），版型不是「商品」時這格不用管
             O 欄＝文案內容（常是「文案最多5字內」這種佔位提示字，不是真的文案，
                    見 isPlaceholderCaption()） */
    if(current && labelVal === '直播間FL'){
      var flVariantVal = String(row[13] || '').trim(); // 同一列的 N 欄
      var nextRow = rows[ri+1];
      console.log('[import][FL] 第'+(ri+1)+'列 labelVal=直播間FL，flVariantVal='+JSON.stringify(flVariantVal)+
        '，nextRow(M/N/O)='+JSON.stringify(nextRow ? [nextRow[12],nextRow[13],nextRow[14]] : null));
      if(flVariantVal){
        if(flVariantVal.indexOf('不製作') !== -1){
          current.flProductSlot = 'skip';
        } else if(flVariantVal.toUpperCase() === 'LOGO'){ // 原本是精準比對'LOGO'，改成不分大小寫/避免多打空白就抓不到
          current.flProductSlot = 'logo';
          /* 下一列的N欄填的不是「商品1/商品2/商品3」這種代碼，是這個LOGO實際的
             名稱（拿去跟素材資料夾比對用），跟下面 flProductNameMap 那個「選商品」
             的 N 欄意義不同，這裡直接整串當名稱塞進 logoName（一般 LOGO 欄位掃描
             抓不到時，這裡補上）。只在還沒有 logoName（or 是數字/字面上就是"LOGO"）
             時才覆蓋，避免蓋掉前面已經從別處掃到的正確值。 */
          if(nextRow){
            var flLogoNameRaw = String(nextRow[13] || '').trim();
            console.log('[import][FL] 版型=LOGO，讀到的N欄(logo名稱候選)='+JSON.stringify(flLogoNameRaw));
            if(flLogoNameRaw && flLogoNameRaw.toUpperCase() !== 'LOGO' && (!current.logoName || current.logoName.toUpperCase() === 'LOGO')){
              current.logoName = flLogoNameRaw;
            }
          }
          /* 跟手動切換「直播間FL ICON」下拉選單同一套規則對齊（見 editor-utils.js
             handleFlSlotChange）：版型=LOGO 時 flText 欄位本身也要是字串"logo"，
             07_fl.html 判斷要不要畫 Logo 版型看的正是這個值，不是 flProductSlot。
             這裡明確設定，下面的文案欄位（O欄）不會再覆蓋過去。 */
          current.flText = 'logo';
        } else if(flVariantVal.indexOf('商品') !== -1 && nextRow){
          var flProductNameMap = { '商品1':'1', '商品2':'2', '商品3':'3', '禮物盒':'gift' };
          var productRaw = String(nextRow[13] || '').trim(); // N 欄
          if(flProductNameMap[productRaw] !== undefined){
            current.flProductSlot = flProductNameMap[productRaw];
          }
        } else if(flVariantVal.indexOf('純案型') !== -1){
          current.flProductSlot = ''; // 純文案，不需要商品
        }
      }
      /* 文案：下一列的 O欄（原本誤以為是P/Q）。版型=LOGO時上面已經強制設成"logo"，
         這裡不會再覆蓋；佔位提示字（「文案最多5字內」之類）也會被過濾掉，不當真文案存入 */
      if(nextRow && current.flProductSlot !== 'logo'){
        var flTextVal = nextRow[14];
        if(flTextVal && !isPlaceholderCaption(flTextVal)) current.flText = String(flTextVal).trim();
      }
    }
  });
...
function applyFieldMap(fm){
  ...
  /* 底色吸取（毛孩衝蝦米）：M/N欄的「背景色」／「背景色碼」／「指定色號」／「指定色碼」
     過濾「無/GD/請/指定」等無效填寫（沿用 smt直播間 editor-import.js 同一組過濾字樣），
     正常填一個hex色碼才會套用，套用後LPBN/縮圖/開播字卡/FL Icon/Tab的背景跟公版文字色都會自動重算 */
  var bgRaw = fs(fm['背景色']) || fs(fm['背景色碼']) || fs(fm['指定色號']) || fs(fm['指定色碼']);
  if(bgRaw && !/^(無|GD|請|指定)/.test(bgRaw)){
    var normalizedBg = window.ColorThemeEngine && window.ColorThemeEngine.normalizeHex(bgRaw);
    if(normalizedBg){ applySeedHex(normalizedBg); filled++; }
  }
...
function applyExcelData(rows){
  ...
  /* 底色吸取（毛孩衝蝦米）：同 applyFieldMap()，過濾「無/GD/請/指定」等無效填寫 */
  var bgRaw2 = getVal(fieldMap['背景色'] || fieldMap['背景色碼'] || fieldMap['指定色號'] || fieldMap['指定色碼'] || '');
  if(bgRaw2 && !/^(無|GD|請|指定)/.test(bgRaw2)){
    var normalizedBg2 = window.ColorThemeEngine && window.ColorThemeEngine.normalizeHex(bgRaw2);
    if(normalizedBg2){ applySeedHex(normalizedBg2); filled++; }
  }
...
function importZip(file){
  ...
  JSZip.loadAsync(file).then(function(zip){
    ...
    return xlsxFile.async('arraybuffer').then(function(buf){
      ...
      if(hostName){
        try{
          var hostLib = JSON.parse(localStorage.getItem('bn_hosts_pet_frenzy_v1')||'[]');
