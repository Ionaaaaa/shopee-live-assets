...
(function(global){
  ...
  ready(function(){
    ...
    /* 等 sidebar 出現 */
    /* 這裡原本會呼叫 insertSaveLoadBar()，在sidebar左下角另外插一組「⬇下載暫存／
       ⬆上傳暫存」按鈕──但頂部工具列本來就有「儲存暫存／載入暫存」，功能完全重複
       （而且頂部那組用buildStateBlob()存的資料比較完整，含TABS/ACTIVE_TAB），
       所以移除這裡多插的一份，只保留自動存檔/自動讀檔（autoSave／autoLoad）跟
       banwords，不影響其他功能。 */
    function tryInsert(){
