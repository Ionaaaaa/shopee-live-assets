...
(function () {
  ...
  // 對應公版工單 M28:R32
  /* 版型組合改成3種（2026-07：拿掉「2人+1品」，因為工單「商品組合」下拉選單
     本來就只有A組合(2人)／B組合(1人+2品)／C組合(3品)這3個選項，「2人+1品」
     從來沒有Excel工單會選到，只是後台手動下拉選單裡多餘的選項，直接刪掉。
     字母跟著往前遞補：原本 C(1人+2品) 改成 B、原本 D(3品) 改成 C，
     跟Excel「商品組合」欄位的A/B/C字母意義完全一致了，之後
     js/editor-import.js 的 mapExcelComboToShadowCombo() 也不用再另外
     轉換字母。 */
  var COMBOS = { // enabled 陣列順序＝預設疊放順序：陣列前面＝後方，後面＝前方
    A: { label: 'A組合（2人）', enabled: ['host2', 'host1'] },                    // 人物1最前面
    B: { label: 'B組合（1人+2品）', enabled: ['product1', 'product2', 'host1'] }, // 人物1最前，商品2在商品1前面
    C: { label: 'C組合（3品）', enabled: ['product1', 'product2', 'product3'] }   // 商品3（取代人物1位置）最前面，跟B組合人物1最前面的邏輯一致
  };
  ...
  var hardResetListeners = []; // 跟 changeListeners 分開：只在 restoreState()（真的換了一份完全不同的資料，
                                // 例如切分頁）時通知，給外部知道「這次該整個清空重建，不是小幅度調整」，
                                // 避免外部把普通的點選/單一素材增減也誤判成整批換資料處理
  ...
  window.ShadowEditor = {
    ...
    setSlotFromFile: loadSlotFile,       // setSlotFromFile(slotId, fileObj, ratio?) → ratio是比例（0~3，1=100%），來自Excel「(商品)比例」欄位，選填
    ...
    /* 給 1200x1200 大畫布訂閱：只有 restoreState()（切分頁/整批還原）才會呼叫 cb()，
       用來跟上面 onStateChange 的「小幅度變更」區分開，該整個清空重建時才清空重建 */
