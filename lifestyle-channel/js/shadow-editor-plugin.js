/*
  ...
  3. 素材命名為 host1 / host2 / product1 / product2 / product3，
     對應公版工單 M20:M24，方便之後 Excel／資料夾自動化直接寫入
...
(function () {
  ...
  // 對應公版工單 M20:M24
  var SLOT_DEFS = [
    { id: 'host1', label: '主持人1', type: 'person' },
    { id: 'host2', label: '主持人2', type: 'person' },
  ...
  var COMBOS = { // enabled 陣列順序＝預設疊放順序：陣列前面＝後方，後面＝前方
    A: { label: 'A組合（2人）', enabled: ['host2', 'host1'] },                    // 主持人1最前面
    B: { label: 'B組合（2人+1品）', enabled: ['product1', 'host2', 'host1'] },    // 主持人1最前，主持人2其次，商品在最後方
    C: { label: 'C組合（1人+2品）', enabled: ['product2', 'product1', 'host1'] }, // 主持人1最前，商品在後方
    D: { label: 'D組合（3品）', enabled: ['product3', 'product2', 'product1'] }   // 商品1最前面
  };
    ...
    return '' +
      '<div class="sg" id="lc-sg">' +
        '<div class="sg-body">' +
          ...
          '</div>' +
          '<div class="lc-field">' +
            '<label>素材（可拖曳排序、右上角 × 刪除；Ctrl/⌘/Shift+點擊可多選，多選後畫布可整組拖曳／縮放）</label>' +
  ...
  function sendFullStateTo(ifr){
    ...
    SLOT_DEFS.forEach(function(def){
      if (state.slots[def.id]){
        w.postMessage({ type:'LC_UPSERT_SLOT', slotId: def.id, slotType: def.type, dataUrl: state.slots[def.id] }, '*');
      }
    });
    w.postMessage({ type:'LC_SET_ENABLED', ids: state.order }, '*');
  }

  function broadcastEnabled(){
    broadcastToAllFrames({ type:'LC_SET_ENABLED', ids: state.order });
  }
  ...
  function applySlotDataUrl(slotId, dataUrl){
    var def = SLOT_DEFS.filter(function(d){ return d.id===slotId; })[0];
    if (!def) return;
    state.slots[slotId] = dataUrl;
    broadcastToAllFrames({ type:'LC_UPSERT_SLOT', slotId: slotId, slotType: def.type, dataUrl: dataUrl });
    setSelection([slotId]);
    renderSlotBar();
    notifyChange();
  }

  function loadSlotFile(slotId, file){
    var reader = new FileReader();
    reader.onload = function(ev){ applySlotDataUrl(slotId, ev.target.result); };
  ...
  window.ShadowEditor = {
    ...
    setSlotFromFile: loadSlotFile,       // setSlotFromFile(slotId, fileObj)
