...
window.ShadowLayoutReceiver = (function () {
  ...
  function create(canvas){
    ...
    function upsertSlot(slotId, type, dataUrl, redraw){
      var img = new Image();
      img.onload = function(){
        ShadowPlugin.registerProduct(slotId, img, type).then(function(){
          ...
          if (!slots[slotId]){
            var layout = DEFAULT_LAYOUT[slotId];
            var h0, w0, x, y;
            if (layout){
              var placed = null;
            ...
            slots[slotId] = { x: x, y: y, w0: w0, h0: h0, scaleMul: 1, tight: tight };
          } else {
    ...
    function handleMessage(msg, redraw){
      ...
      switch (msg.type){
        ...
        case 'LC_SET_ANGLE':
          ...
          break;
        case 'LC_UPSERT_SLOT':
          upsertSlot(msg.slotId, msg.slotType, msg.dataUrl, redraw);
          break;
        case 'LC_REMOVE_SLOT':
