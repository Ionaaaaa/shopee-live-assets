/*
  ...
  3. 素材命名為 host1 / host2 / product1 / product2
     （host1＝主持人，host2＝來賓），對應公版工單 M20:M23，方便之後 Excel／資料夾自動化直接寫入
...
(function () {
  'use strict';
  console.log('%c[shadow-editor-plugin.js] 版本確認：2026-07-22-v4（C組合改2品，拿掉product3）', 'background:#222;color:#0f0;font-weight:bold;padding:2px 6px;');
  ...
  /* 版型組合3種（2026-07：C組合這個專案實際上永遠只會用到2品，商品3從沒被
     用過，直接把product3這個代號整個拿掉，C組合改標「2品」，跟Excel實際
     填寫狀況一致。之前「2人+1品」拿掉、字母遞補到A/B/C對齊Excel欄位的
     調整維持不變。） */
  var COMBOS = { // enabled 陣列順序＝預設疊放順序：陣列前面＝後方，後面＝前方
    A: { label: 'A組合（2人）', enabled: ['host2', 'host1'] },                    // 主持人最前面
    B: { label: 'B組合（1人+2品）', enabled: ['product1', 'product2', 'host1'] }, // 主持人最前，商品2在商品1前面
    C: { label: 'C組合（2品）', enabled: ['product1', 'product2'] }               // 純商品，商品2在商品1前面
  };
  ...
  var hardResetListeners = []; // 跟 changeListeners 分開：只在 restoreFullState()（真的換了一份完全不同的資料，
                                // 例如切分頁／批次匯入下一個廠商）時通知，給外部知道「這次該整個清空重建，
                                // 不是小幅度調整」，避免外部把普通的點選/單一素材增減也誤判成整批換資料處理。
                                // （這個機制原本就設計好了，但 window.ShadowEditor 的對外 API 一直沒有實際
                                // 註冊 onHardReset／在 restoreFullState() 裡呼叫，導致 editor-shadow-canvas.js
                                // 裡訂閱這個事件的程式碼形同虛設——結果是：批次匯入多個廠商、或切分頁時，
                                // 1200大畫布內部快取的商品位置/尺寸(w0/h0)不會真的清空，下一個廠商的商品
                                // 圖檔換了新的長寬比例，卻繼續套用「上一個廠商那張圖」算出來的尺寸，
                                // 畫面上就會看到商品被硬套進不合比例的框裡、變形拉長。）
  ...
  function loadSlotFile(slotId, file, ratio, cb){
    ...
    reader.onerror = function(){ if(cb) cb(false); };
  ...
  /* 完整還原一份先前存下來的 snapshot（getFullState() 的輸出）。
     專為「一個廠商=一個分頁，彼此不能互相干擾」設計：
     切分頁時把目前分頁的 slots/combo/order 存起來，換到下一個分頁時
     用這支整包蓋回去，不會殘留上一個廠商的商品／主持人圖。
     pendingMode 底下呼叫的話跟其他操作一樣，只更新本地，不廣播出去，
     等外部呼叫 commit() 才送給畫布。 */
  function restoreFullState(snap){
    if(!snap) return;
    /* 通知訂閱者（1200x1200大畫布）：這次是整包換一份完全不同的資料
       （切分頁／批次匯入下一個廠商），要整個清空重建，不能沿用舊資料
       算出來的商品位置/尺寸——這一行原本漏掉，是造成商品匯入後變形的
       實際原因，詳見上面 hardResetListeners 宣告處的說明。 */
    hardResetListeners.forEach(function(cb){ try{ cb(); }catch(e){ console.warn('shadow-editor-plugin onHardReset callback error:', e); } });
    state.combo = snap.combo || state.combo;
    state.angle = snap.angle || state.angle;
    state.bgDataUrl = snap.bgDataUrl || null;
    state.slots = Object.assign({}, snap.slots || {});
    state.slotRatios = Object.assign({}, snap.slotRatios || {});
    state.polaroid = Object.assign({}, snap.polaroid || {});
    state.slotOriginal = Object.assign({}, snap.slotOriginal || {});
    ...
    if(!pendingMode){
      ...
      Object.keys(frames).forEach(function(id){
        var ifr = frames[id];
        /* 先請每個版位清空它自己記憶體裡的商品插槽，再送完整狀態重建——避免切分頁時
           上一個廠商留下、這次沒提到的插槽卡在畫面上（sendFullStateTo 只會送
           「這次有值」的插槽，不會主動清掉舊的） */
  ...
  window.ShadowEditor = {
    ...
    setCombo: setCombo,             // setCombo('A'|'B'|'C')
    ...
    setFullState: restoreFullState,  // 分頁切換用：整包還原 getFullState() 存下的 snapshot
    ...
    /* 真的整批換一份資料（切分頁／批次匯入下一個廠商）才會通知，見上面
       hardResetListeners 宣告處的說明；跟 onStateChange 分開，只有這種
       情境才觸發，避免一般的點選/單一素材增減也被誤判成整批換資料。 */
    onHardReset: function(cb){ if(typeof cb === 'function') hardResetListeners.push(cb); },
