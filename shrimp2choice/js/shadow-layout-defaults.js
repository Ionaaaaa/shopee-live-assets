/*
  ...
  window.ShadowLayoutDefaults = {
    ...
    C: { product1:{...}, product2:{...} }                  // 2品
  };
  每個版型只列出「這個版型實際會用到」的代號（例如 B 沒有 host2、C 完全沒有 host），
  一個版型的數字只會影響那個版型，改 B 不會動到 A/C，之後要加新版型／新內容，
  直接照這個格式在最外層加一組新的版型代號即可，其他版型完全不受影響。

  （2026-07 更新：拿掉了「2人+1品」這個版型，因為 Excel 工單「商品組合」欄位
  本來就只有2人／1人+2品／3品這3個選項，「2人+1品」從來沒被用過。字母跟著
  往前遞補：原本 C(1人+2品) 改成 B、原本 D(3品) 改成 C，現在跟 Excel 欄位
  A/B/C 字母意義完全一致，workorder-parser-shrimp.js 的 INTERNAL_COMBO_SLOTS
  也已經同步改成這套字母。）
...
(function(){
  ...
  var PRODUCT_SCALE_UP    = 2;    // 商品整體再放大的倍率（商品在1200畫布裡預設偏小，
  ...
  window.ShadowLayoutDefaults = {
    ...
    // ── B：1人＋2品，三角形擺放：人偏左、商品1置中、商品2偏右下（原本的C組合，只是改叫B，數字完全沒動） ──
    B: {
    ...
    // ── C：2品，完全沒有人物（這個專案C組合實際上永遠只會用到2品，商品3
    //     從沒被用過，已整個拿掉）。跟B組合同一種對角擺法（左上偏後、右下偏前），
    //     原本B組合這組數字是設計給「1人+2品」用的，人物在左邊撐住視覺重心，
    //     商品1/商品2整組因此偏右（xPct 0.58／0.74，平均0.66）；C組合沒有人物，
    //     兩品自己要能大致置中，所以整組往左移0.16，讓平均值落回0.5：
    //     商品1 0.58→0.42、商品2 0.74→0.58，兩者間的相對位置差、yPct、hPct都不變。 ──
    C: {
      product1: { xPct: 0.42, yPct: 0.62, hPct: 0.36 * PRODUCT_SCALE_UP }, // 左上，兩品置中後的主商品
      product2: { xPct: 0.58, yPct: 0.70, hPct: 0.28 * PRODUCT_SCALE_UP }  // 右下，疊在商品1前面
    },

    /* 備援：理論上不會用到——只有在收不到目前版型資訊時（例如訊息順序異常）才會退回這裡，
       避免完全沒有位置可用。維持改版前的既有數字，4個代號都給一個合理位置。 */
    _fallback: {
