...
function buildHostPopup(){
  ...
  function renderHosts(hosts){
    ...
    var libStored = [];
    try{ libStored = JSON.parse(localStorage.getItem('bn_hosts_shrimp2choice_v1')||'[]'); }catch(e){}
...
/* 共用：把一個 File 讀成 dataURL 套進 host/logo1/logo2，不管來源是手動選檔還是資料夾批次比對。
   keepPos（預設false/不傳＝原本行為）：true 表示這次只是「內容更新」（例如陰影彈窗裡重新調整
   商品/主持人再次確認匯出），不是「全新素材第一次進來」，會透過 broadcastFull() 的 payload
   一路傳到各版位的 D.imgX/imgY/imgScale，讓版位那邊跳過 initHostPos() 自動貼合、
   保留使用者已經在畫布上手動調整過的位置/大小（版位那邊 isNew 判斷本來就有
   !D.host || 的防呆，這個分頁第一次收到host時keepPos=true也不會漏掉初始定位）。 */
function applyImageFile(file,key,cb,keepPos){
