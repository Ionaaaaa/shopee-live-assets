...
window.ShadowLayoutReceiver = (function () {
  'use strict';
  console.log('%c[shadow-layout-receiver.js] 版本確認：2026-07-21-v3（含版型獨立位置＋商品比例＋旋轉功能）', 'background:#222;color:#0f0;font-weight:bold;padding:2px 6px;');
  ...
  function create(canvas){
    ...
    var _slotGeneration = 0; // 每次 LC_RESET_SLOTS（真正換一批完全不同的資料，例如切分頁）就+1，
                              // upsertSlot() 讀圖是非同步的（new Image()+onload，還會再等
                              // ShadowPlugin.registerProduct() 的 Promise），如果切分頁切得快、
                              // 上一個分頁的圖片還沒載入完成使用者就已經看到下一頁，等上一頁那個
                              // 遲來的 onload 終於觸發時，會誤判「這個slot目前是空的」就把上一頁
                              // 的舊圖寫進「這一頁」的 slots 裡，變成商品跑錯分頁。upsertSlot()
                              // 呼叫當下記住現在是第幾世代，圖片真正載入完成時如果世代已經不一樣
                              // （代表期間又切過分頁），直接丟棄這次結果，不寫入。
    ...
    /* ── 復原（Ctrl+Z）：只記錄「位置/縮放/旋轉」這幾個欄位的微調，不含
       上傳/刪除素材、版型組合切換——範圍刻意縮小，最多存5步，見
       pushUndoSnapshot()／undo()。跟Logo2畫布的復原是各自獨立的兩套堆疊
       （見 editor-canvas-ui.js 的 window.BNUndo 通用分派器），不共用資料，
       其中一邊的復原邏輯出問題不會影響另一邊。 */
    var undoStack = [];
    var UNDO_MAX = 5;
    ...
    // 商品/主持人圖片下緣如果有透明留白，ShadowPlugin 畫照片本體時會把圖整張往下
    // 補回這段留白（見 shadow-plugin.js drawGroundShadow/drawPersonGlow 的 py = state.y + trimBottomPad），
    // 這樣「有顏色的部分」的下緣才會真正貼齊 state.y 這個地面錨點。
    // 選取框／拖曳用的 itemBounds() 如果沒有比照補上同一段位移，算出來的框會比實際畫面
    // 整體高了 trimBottomPad 這麼多（永遠是「往上」而不是隨機方向，因為留白只可能讓框
    // 少往下延伸，不會少往上延伸）——這裡要跟畫圖那邊用同一個公式，兩邊才會對齊。
    function getTrimBottomPad(slotId, fullH){
      var p = (window.ShadowPlugin && ShadowPlugin._products) ? ShadowPlugin._products[slotId] : null;
      return (p && p.trim) ? p.trim.bottom * fullH : 0;
    }
    ...
    function itemBounds(slotId){
      ...
      var trimBottomPad = getTrimBottomPad(slotId, fullH);
    ...
    function upsertSlot(slotId, type, dataUrl, redraw, ratio, savedPos){
      var myGen = _slotGeneration;
      var img = new Image();
      img.onload = function(){
        ShadowPlugin.registerProduct(slotId, img, type).then(function(){
          if (myGen !== _slotGeneration) return; // 期間已經 LC_RESET_SLOTS 切過分頁了，這次結果是舊分頁的，直接丟棄不寫入
          slotType[slotId] = type;
          var tight = calcTightBoundsRatio(img);
          if (!slots[slotId]){
            if (savedPos){
              /* 這個slot是全新建立的（剛被LC_RESET_SLOTS清空過，例如切分頁），
                 且外部（editor-shadow-canvas.js）記得住這一頁上次調整過的位置，
                 直接套用，不要走下面重新計算預設位置那條路。 */
              slots[slotId] = { x: savedPos.x, y: savedPos.y, w0: savedPos.w0, h0: savedPos.h0, scaleMul: savedPos.scaleMul, rot: savedPos.rot || 0, tight: tight };
              if (redraw) redraw();
              return;
            }
            ...
            /* 商品比例（Excel「(商品)比例」欄位）：100% 或沒填就是 1（維持這裡算出來的最大尺寸），
               填了更小的比例就縮小、填了大於100%（最高300%）就放大，都是從第一次貼合的當下
               直接套用；之後使用者拖曳調整過，就照使用者調整的結果。 */
            ...
            /* 換圖時（例如勾選/取消拍立得框，圖片從「原始照片」換成「壓平後的框+照片」，
               ...
               匯入工單時同一個廠商分頁重複比對/覆蓋同一個插槽也算「換圖」）
               一定要用「新圖片本身」的長寬比重算 w0，不能直接沿用舊的 w0！
               舊寫法只更新 tight、完全不動 w0/h0，等於用舊照片的窄長比例框去套新的
               正方形/不同比例圖片，畫出來會被硬擠成細長形——這正是商品匯入後「變細
               變形」的原因之一。這裡固定保留 h0（高度＝使用者原本調整過的視覺大小
               不變），用新圖片的實際比例重新算 w0，兩者相除永遠等於新圖片真正的
               寬高比，不會拉伸。 */
    ...
    function handleMessage(msg, redraw){
      ...
      switch (msg.type){
        case 'LC_RESET_SLOTS':
          /* 完整還原前先清空——upsertSlot() 遇到「這個插槽已經存在」時只會換
             圖片、刻意保留舊的位置/大小（讓使用者手動拖曳調整過的結果不會被
             換圖沖掉）。但這個「保留舊位置」的假設，只在「同一份資料裡換圖」
             時成立；換成另一份完全不同的資料（切分頁）時，舊位置反而是錯的、
             對不上這份新資料，會變成插槽位置卡在不該出現的地方，看起來像
             多了一套商品。清空之後每個插槽都當作「第一次加入」處理，會正確
             套用這份資料自己的預設位置。 */
          _slotGeneration++; // 宣告新的一輪：上一輪還在非同步載入中、還沒完成的 upsertSlot() 回呼晚點回來時會被丟棄
          Object.keys(slots).forEach(function(id){ ShadowPlugin.removeProduct(id); });
          slots = {};
          slotType = {};
          setSelection([]);
          if (redraw) redraw();
        ...
        case 'LC_SET_ANGLE':
          ...
          break;
        case 'LC_UPSERT_SLOT':
          upsertSlot(msg.slotId, msg.slotType, msg.dataUrl, redraw, msg.ratio, msg.savedPos);
          break;
        case 'LC_REMOVE_SLOT':
    ...
    return {
      ...
      /* 給「記住上次畫布位置」這類功能用：回傳每個slot完整的原始資料（x,y,w0,h0,scaleMul,rot,tight），
         跟上面 getOrderedStates() 不同——那個是已經算好 w=w0*scaleMul 的展示用格式，這裡要保留
         原始欄位（尤其是 scaleMul 跟 w0/h0 分開），日後要無損寫回去才不會失真。
         editor-shadow-canvas.js 的 onHardReset 換頁邏輯已經在呼叫這支函式存位置，
         這裡原本沒有實作，等於每次切分頁都存不到東西——一併補上。 */
      getRawSlotStates: function(){
        var out = {};
        Object.keys(slots).forEach(function(id){
          var s = slots[id];
          out[id] = { x:s.x, y:s.y, w0:s.w0, h0:s.h0, scaleMul:s.scaleMul, rot:s.rot||0, tight:s.tight };
        });
        return out;
      },
      ...
      /* 復原（Ctrl+Z）：見上方 pushUndoSnapshot()/undo() 的註解，只涵蓋位置/縮放/旋轉，
         最多5步。peekUndoTs() 給外部的通用復原分派器（window.BNUndo）比較「這個編輯
         系統最後一次動作是什麼時候」用，只有時間最新的那個系統會被復原。 */
      undo: undo,
