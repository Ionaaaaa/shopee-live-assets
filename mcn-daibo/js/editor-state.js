...
/* 每個 key 對應 backgrounds/{key}/ 資料夾，色碼／背景圖規則詳見 themes.js */
function bgBaseFor(t){ return '../backgrounds/'+t+'/'; }
/* 依主題取得 logo1／CTA 圖檔路徑（相對於 layout iframe，所以要補 '../'），
   主題沒填就退回共用預設檔，version.js 可對照著看 */
function themeAssetUrl(t, field, fallback){
  var def = BN_THEME_MAP[t];
  var rel = (def && def[field]) ? def[field] : fallback;
  return '../'+rel;
}

/* ── iframe 清單 ── */
/* LAYOUTS 從後台 localStorage 讀取啟用的版位 */
var STORE_KEY = 'bn_admin_mcn_live_v1';
var DEFAULT_LAYOUTS = [
...
/* v1：MCN代播專案改版，僅保留 01_thumbnail、02_lpbn 兩個版位 */
var DEFAULT_LAYOUTS_VERSION = 1;
...
/* 依 BN_THEMES（js/themes.js）動態產生公版款式下拉選單。
   只需維護 themes.js，這裡不用手動增修任何 key/label；
   之後公版數量增加到十幾組，下拉選單也不需要調整。 */
function renderThemeChips(){
  var sel = document.getElementById('theme-select');
  if(!sel) return;
  sel.innerHTML = '';
  BN_THEMES.forEach(function(th){
...
function setTheme(t){
  var def = BN_THEME_MAP[t];
  if(!def){
    console.warn('[setTheme] 找不到公版 key「'+t+'」，請確認 js/themes.js 有這筆設定');
    return;
  }
  S.theme = t;
  document.getElementById('cSub').value  = def.cSub;
  document.getElementById('cMain').value = def.cMain;
  document.getElementById('cDate').value = def.cDate;
  var elSep = document.getElementById('cSep'); if(elSep) elSep.value = def.sepColor;
  var elBar = document.getElementById('cBar'); if(elBar) elBar.value = def.barColor;
  var elBarText = document.getElementById('cBarText'); if(elBarText) elBarText.value = def.barTextColor || '#1f305d';
  var elBarOpacity = document.getElementById('cBarOpacity'); if(elBarOpacity) elBarOpacity.value = (def.barOpacity===undefined ? 1 : def.barOpacity);
  /* 更新下拉選單目前值＋亮燈狀態（選到某個公版就亮橘燈；若畫面上還沒渲染選單也沒關係） */
  var sel = document.getElementById('theme-select');
  if(sel){ sel.value = t; sel.classList.add('active'); }
  broadcast();
}
...
function collectState(){
  return {
    ...
    colors:{cSub:v('cSub'),cMain:v('cMain'),cDate:v('cDate'),cSep:v('cSep'),cBar:v('cBar'),cBarText:v('cBarText'),cBarOpacity:v('cBarOpacity')},
...
function applyState(data){
  ...
  if(data.colors){ set('cSub',data.colors.cSub);set('cMain',data.colors.cMain);set('cDate',data.colors.cDate);set('cSep',data.colors.cSep);set('cBar',data.colors.cBar);set('cBarText',data.colors.cBarText);set('cBarOpacity',data.colors.cBarOpacity); }
  if(data.logo2Edit){
