...
function parseWorkorderGroups(rows){
  ...
  var fieldMap = {'款式':'theme','主標':'main','副標':'sub','日期':'date','時間':'time','素材路徑':'hostPath','版型':'combo',
    ...
    '主持人':'host1Name','來賓':'host2Name', /* 舊版工單欄位相容（沒有「購物專家」欄位時的舊格式） */
  ...
  rows.forEach(function(row){
    ...
    /* 公版工單同一張表裡「版型」出現兩次，意義不同，用同一列的 A 欄來分辨：
       第一次出現在「檔名」表頭那一列（M12/N12）── 這是選公版款式（對應 backgrounds/ 資料夾 + themes.js 色票），
       第二次出現在下面內容清單裡（M18/N18）── 這是 ABCD 人物/商品組合版型，兩者不能共用同一個 key */
    if(current && mVal === '版型' && aVal === '檔名' && nVal !== undefined && nVal !== null && nVal !== ''){
      current.theme = String(nVal).trim();
      return;
    }
