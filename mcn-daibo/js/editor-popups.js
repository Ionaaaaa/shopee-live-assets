...
/* ── Popup ── */
/* 背景款式清單改由 js/themes.js 的 BN_THEMES 提供，不再寫死於此檔 */
...
function buildBgPopup(){
  ...
  body.innerHTML = '';
  BN_THEMES.forEach(function(opt){
    var card = document.createElement('div');
    card.className = 'popup-card' + (S.theme===opt.key ? ' selected' : '');
    card.innerHTML =
      '<div style="height:60px;background:'+opt.cMain+';border-radius:4px 4px 0 0;overflow:hidden;">'
      +(opt.preview ? '<img src="'+opt.preview+'" style="width:100%;height:100%;object-fit:cover;" onerror="this.style.display=\'none\'">' : '')
      +'</div>'
...
function buildHostPopup(){
  ...
  function renderHosts(hosts){
    ...
    var libStored = [];
    try{ libStored = JSON.parse(localStorage.getItem('bn_hosts_mcn_live_v1')||'[]'); }catch(e){}
