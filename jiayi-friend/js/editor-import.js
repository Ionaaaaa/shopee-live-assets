...
function parseWorkorderGroups(rows){
  ...
  rows.forEach(function(row, ri){
    ...
    /* FL 版型／商品／文案：這份工單格式裡沒有「FL文案(6字)」這個 M/N 欄位標籤可以比對，
       改成直接讀「直播間FL」這個跨兩列的合併區塊（M19:M20），已經跟 Iona 確認過結構：
         - 第一列（M='直播間FL' 那一列）的 O／P／Q 欄（合併成一格）＝版型，是決定一切的
           主欄位，例如「LOGO」／「商品 + 案型5字內」／「純案型6字內」／「不製作」
         - 第二列（緊接在下面那一列）：
             N 欄＝版型是「商品 + 案型5字內」時，選的是哪個商品（商品1／商品2／商品3／禮物盒）
             O 欄＝也叫「LOGO」，但版型欄位才是主要依據——版型不是「LOGO」時，
                    這一格就算剛好殘留「LOGO」字樣也不用管它（已跟 Iona 確認過）
             P／Q 欄（合併）＝文案內容 */
    if(current && mVal === '直播間FL'){
      var flVariantVal = String(row[14] || row[15] || row[16] || '').trim(); // O/P/Q，同一列合併儲存格
      var nextRow = rows[ri+1];
      if(flVariantVal){
        if(flVariantVal.indexOf('不製作') !== -1){
          current.flProductSlot = 'skip';
        } else if(flVariantVal === 'LOGO'){
      ...
      if(nextRow){
        var flTextVal = nextRow[15] || nextRow[16]; // 下一列的 P/Q（合併儲存格）
        if(flTextVal) current.flText = String(flTextVal).trim();
      }
    }
  });
