...
window.ShadowLayoutReceiver = (function () {
  'use strict';
  console.log('%c[shadow-layout-receiver.js] 版本確認：2026-07-06-v2（含版型獨立位置＋商品比例功能＋旋轉/多選/復原）', 'background:#222;color:#0f0;font-weight:bold;padding:2px 6px;');

  /* 旋轉解耦：旋轉「只」影響最後畫到 canvas 上的視覺效果（照片+貼地陰影/光暈一起轉），
     完全不影響 itemBounds／拖曳／縮放控制點的判定邏輯——這些全部維持軸對齊矩形計算，
     跟原本沒有旋轉功能時一模一樣。實際畫出旋轉效果的地方在 shadow-plugin.js
